...
class Course(models.Model):
    ...
    period = models.IntegerField(verbose_name="建议学习周期(days)",null=True)
...
class CourseSection(models.Model):
    ...
    name = models.CharField(max_length=128)
    order = models.SmallIntegerField(verbose_name="课时排序", help_text="建议每个课时之间空1至2个值，以备后续插入课时")
    ...
    free_trail = models.IntegerField(verbose_name='1可以观看，0不可以观看')
...
class Homework(models.Model):
    ...
    order = models.SmallIntegerField("作业顺序", help_text="同一课程的每个作业之前的order值间隔1-2个数")
    ...
    recommend_period = models.SmallIntegerField("推荐完成周期(天)", default=7)
    scholarship_value = models.SmallIntegerField("为该作业分配的奖学金(贝里)")
